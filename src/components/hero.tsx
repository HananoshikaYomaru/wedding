...
import { Heart, Upload, X, LoaderCircle, Crop, Image } from "lucide-react";
import "./hero.css";
...
// Image segmentation worker setup
const setupImageSegmentationWorker = () => {
...
const ImageUploadDialog = ({
  ...
  useEffect(() => {
    if (isOpen) {
      try {
        workerRef.current = setupImageSegmentationWorker();
  ...
  const handleSegmentationResult = (data: any) => {
    // In a real implementation, we would process the mask data here
    // For now, we'll simulate a segmented image
    if (selectedImage) {
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    const newPoint = {
      x,
      y,
      label: e.button === 0 ? 1 : 0, // Left click = foreground, right click = background
    };

    setPoints([...points, newPoint]);
    setIsProcessing(true);

    // In a real implementation, we would send this to the worker
  ...
  return (
    <div className="segmentation-dialog">
      <div className="dialog-header">
        <h3>Create Your Sticker</h3>
        ...
        {!selectedImage ? (
          ...
          <>
            <p className="dialog-instructions">
              Click on the object you want to turn into a sticker
            </p>
...
const DraggableHearts = () => {
  ...
  const handleStickerDragEnd = (info: any, id: number) => {
    ...
    <div className="hearts-container">
      ...
      {stickers.map((sticker) => (
        <motion.div
          ...
          onDragEnd={(_, info) => handleStickerDragEnd(info, sticker.id)}
    ...
    <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden ">
      ...
      <div className="relative z-10 mt-12 w-full max-w-md h-48 border-2 border-dashed border-[#d3b8a3] rounded-lg flex items-center justify-center bg-white/50 backdrop-blur-sm">
        <p className="absolute top-2 left-0 right-0 text-center text-[#6d6d6d] text-sm">
          Drag hearts around or add more stickers!
        </p>
