...
export function sliceTensor(
  tensor: { dims: number[]; cpuData: Float32Array },
  idx: number
) {
...
export function float32ArrayToCanvas(
  ...
  for (let srcIdx = 0; srcIdx < array.length; srcIdx++) {
    ...
    // TODO: check if this is correct because maskedPx is boolean
    // @ts-ignore
...
export function canvasToFloat32Array(canvas: HTMLCanvasElement) {
  ...
  const [redArray, greenArray, blueArray] = [[], [], []] as number[][];
...
export function maskCanvasToFloat32Array(canvas: HTMLCanvasElement) {
  ...
  for (let i = 0; i < float32Array.length; i++) {
    float32Array[i] =
      (imageData[i] + imageData[i + 1] + imageData[i + 2]) / (3 * 255.0); // convert avg to float
  }
